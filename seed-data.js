...
function block(title, time, tone) {
  return { title: title, time: time, tone: tone || "normal" };
}
    ...
    rows: DATES_9_04_TO_15.map(function (d) {
      if (d === "09.09") {
        return {
          date: d,
          blocks: [
            block("Grand City / Mercure", "14:00 (for Technical Meeting, Opening Ceremony) - Mercure", "red"),
            block("Grand City / Mercure", "15:00", "blue")
          ]
        };
      }
      return {
        ...
        blocks: TO_CITY_TIMES[d].map(function (time) {
          return block("Grand City / Mercure", time);
        })
      };
    })
  }
